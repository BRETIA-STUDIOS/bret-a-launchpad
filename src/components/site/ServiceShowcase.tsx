...
function ServiceVisual({ kind, active }: { kind: number; active: boolean }) {
    ...
    <svg viewBox="0 0 200 120" aria-hidden="true" fill="none" className="h-full w-full">
      ...
      {kind === 0 && (
        <g className={cn("method-draw", active && "is-active")}>
          <rect x="20" y="40" width="76" height="56" rx="5" stroke={stroke} strokeWidth="1.5" pathLength={1} />
          <rect x="108" y="40" width="72" height="14" rx="3" stroke={faint} strokeWidth="1.5" pathLength={1} />
          <rect x="108" y="62" width="72" height="9" rx="3" stroke={faint} strokeWidth="1.5" pathLength={1} />
          <rect x="108" y="79" width="44" height="9" rx="3" stroke={stroke} strokeWidth="1.5" pathLength={1} />
          <circle cx="58" cy="68" r="3" fill={stroke} className="method-dot" style={{ animationDelay: "480ms" }} />
        </g>
      )}
      ...
      {kind === 1 && (
        <g className={cn("method-draw", active && "is-active")}>
          <path d="M20 44 H84 M20 56 H74 M20 68 H88 M20 80 H66 M20 92 H80" stroke={faint} strokeWidth="1.5" strokeDasharray="4 5" pathLength={1} />
          <path d="M100 36 V100" stroke={faint} strokeWidth="1" />
          <rect x="118" y="42" width="62" height="26" rx="4" stroke={stroke} strokeWidth="1.5" pathLength={1} />
          <path d="M118 80 H180 M118 92 H150" stroke={stroke} strokeWidth="1.5" strokeLinecap="round" pathLength={1} />
          <circle cx="100" cy="68" r="3" fill={stroke} className="method-dot" style={{ animationDelay: "360ms" }} />
        </g>
      )}
      ...
      <div className={cn("lg:col-span-5", flip && "lg:order-1")}>
        <div className="surface-card w-full p-3 sm:p-4">
          <div className="aspect-[5/3] w-full">
            <ServiceVisual kind={visual} active={active} />
          </div>
        </div>
      </div>
    </article>
